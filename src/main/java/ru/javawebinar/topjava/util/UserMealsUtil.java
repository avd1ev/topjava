...
public class UserMealsUtil {
    ...
    public static List<UserMealWithExcess> filteredByCycles(List<UserMeal> meals, LocalTime startTime, LocalTime endTime, int caloriesPerDay) {
        ...
        meals.forEach(um -> caloriesSumPerDate.merge(um.getDate(), um.getCalories(), Integer::sum));
        ...
        for (UserMeal um : meals) {
            if (TimeUtil.isBetweenHalfOpen(um.getTime(), startTime, endTime)) {
                mealExceeded.add(new UserMealWithExcess(um.getDateTime(), um.getDescription(), um.getCalories(),
                        caloriesSumPerDate.get(um.getDate()) > caloriesPerDay));
            }
        }
        return mealExceeded;
    }
